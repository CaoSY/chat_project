$(document).ready(function () {
	...
	$('#send').click(function () {
		...
		if (content.match(/^[<br>|<br/>|<br />]*$/)) {
			...
			addMessageToList(createMessageIn(msgOut));
		}
	});
});
...
function createListItem(item) {
	return `<div class="contact-list-item" id="${item.ID}">
						<div class="avatar">
							<img src="img/${item.imgSrc || 'default-user-image.svg'}" class="item-img img-circle center-block" />
						</div>
						<div class="item-body">
							<div class="item-title">
								<p class="item-name">${item.name || 'anonymous'}</p>
								<span class="item-time">${formatDate(item.timestamp || Date.now())}</span>
							</div>
